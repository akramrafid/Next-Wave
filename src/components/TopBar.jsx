import React, { useEffect, useState } from 'react';

const socials = [
  {
    label: 'Facebook',
    href: 'https://facebook.com/nextwave360',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" width="13" height="13">
        <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" />
      </svg>
    ),
  },
  {
    label: 'Instagram',
    href: 'https://instagram.com/nextwave360',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="13" height="13">
        <rect x="2" y="2" width="20" height="20" rx="5" ry="5" />
        <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
        <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" />
      </svg>
    ),
  },
  {
    label: 'LinkedIn',
    href: 'https://linkedin.com/company/nextwave360',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" width="13" height="13">
        <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-4 0v7h-4v-7a6 6 0 0 1 6-6zM2 9h4v12H2z" />
        <circle cx="4" cy="4" r="2" />
      </svg>
    ),
  },
  {
    label: 'YouTube',
    href: 'https://youtube.com/@nextwave360',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" width="13" height="13">
        <path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46A2.78 2.78 0 0 0 1.46 6.42 29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20.06 12 20.06 12 20.06s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.96A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z" />
        <polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" fill="#060606" />
      </svg>
    ),
  },
];

const tickerItems = [
  'Strategy · Creative · Digital · Execution',
  'Founded in Bangladesh · Built with Global Ambition',
  'Marketing that Moves Business Forward',
  'House 66, Road 06, O. R. Nizam R/A, Chittagong',
  'nextwaves.agency',
];

// Quadruple repetition to ensure seamless width on all wide displays
const itemsBatch = [...tickerItems, ...tickerItems, ...tickerItems, ...tickerItems];

const TopBar = () => {
  const [hidden, setHidden] = useState(false);

  useEffect(() => {
    const onScroll = () => setHidden(window.scrollY > 40);
    window.addEventListener('scroll', onScroll, { passive: true });
    return () => window.removeEventListener('scroll', onScroll);
  }, []);

  return (
    <div
      style={{
        position: 'fixed',
        top: 0,
        left: 0,
        right: 0,
        zIndex: 60,
        height: '32px',
        background: '#060606',
        borderBottom: '1px solid rgba(255,255,255,0.08)',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'space-between',
        padding: '0 20px',
        overflow: 'hidden',
        transform: hidden ? 'translateY(-100%)' : 'translateY(0)',
        transition: 'transform 0.35s cubic-bezier(0.4, 0, 0.2, 1)',
        willChange: 'transform',
      }}
    >
      <style>{`
        @keyframes tickerScrollLeftToRight {
          0%   { transform: translate3d(-50%, 0, 0); }
          100% { transform: translate3d(0%, 0, 0); }
        }
        .topbar-ticker-track {
          display: flex;
          width: max-content;
          animation: tickerScrollLeftToRight 45s linear infinite;
          white-space: nowrap;
          will-change: transform;
        }
        .topbar-social-btn {
          display: flex;
          align-items: center;
          justify-content: center;
          width: 22px;
          height: 22px;
          border-radius: 50%;
          color: rgba(255,255,255,0.6);
          background: rgba(255,255,255,0.06);
          border: 1px solid rgba(255,255,255,0.1);
          transition: all 0.2s ease;
          text-decoration: none;
          flex-shrink: 0;
        }
        .topbar-social-btn:hover {
          background: #0E9ED9;
          color: #fff;
          transform: scale(1.1);
          border-color: #0E9ED9;
        }
      `}</style>

      {/* Continuously flowing ticker moving left to right with no stuck feeling */}
      <div style={{
        flex: 1,
        overflow: 'hidden',
        maskImage: 'linear-gradient(to right, transparent 0%, black 4%, black 96%, transparent 100%)',
        WebkitMaskImage: 'linear-gradient(to right, transparent 0%, black 4%, black 96%, transparent 100%)',
        pointerEvents: 'none',
      }}>
        <div className="topbar-ticker-track">
          {[...itemsBatch, ...itemsBatch].map((item, i) => (
            <span
              key={i}
              style={{
                fontSize: '11px',
                color: 'rgba(255,255,255,0.75)',
                fontFamily: "'DM Sans', sans-serif",
                letterSpacing: '0.04em',
                padding: '0 28px',
                display: 'inline-flex',
                alignItems: 'center',
                gap: '8px',
                flexShrink: 0,
              }}
            >
              {item}
              <span style={{ color: '#0E9ED9', fontSize: '8px' }}>◆</span>
            </span>
          ))}
        </div>
      </div>

      {/* Social icons */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', paddingLeft: '16px', flexShrink: 0 }}>
        <div style={{ width: '1px', height: '12px', background: 'rgba(255,255,255,0.15)', marginRight: '6px' }} />
        {socials.map((s) => (
          <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
            aria-label={s.label} className="topbar-social-btn" title={s.label}>
            {s.icon}
          </a>
        ))}
      </div>
    </div>
  );
};

export default TopBar;
